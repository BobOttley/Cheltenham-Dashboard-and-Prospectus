...
(function() {
    ...
    const CONFIG = {
        ...
        VISIBILITY_THRESHOLD: 0.1,   // 10% of module must be visible (INITIAL)
        REENTRY_THRESHOLD: 0.2,      // 20% required for RE-ENTRY (stricter)
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                ...
                if (entry.isIntersecting) {
                    ...
                    // For re-entries, require 20% visibility (stricter)
                    // For first entry, 10% is fine
            ...
            root: null,
            threshold: [0.1, 0.15, 0.2, 0.25, 0.3], // Multiple thresholds to catch re-entry
            rootMargin: '0px'
        });
    ...
    function setupInteractionTracking() {
        ...
        // Track house expansions
        document.addEventListener('click', (e) => {
            // House system - expand button
            if (e.target.classList.contains('expand-button')) {
                const card = e.target.closest('.house-card');
                const houseName = card?.querySelector('.house-name')?.textContent || 'unknown';
                trackInteraction('house_system', 'house_expand', houseName);
            }

            // House system - mute button
            if (e.target.classList.contains('mute-button')) {
                const card = e.target.closest('.house-card');
                const houseName = card?.querySelector('.house-name')?.textContent || 'unknown';
                const isMuting = !card?.querySelector('.house-video')?.muted;
            ...
            if (e.target.closest('.pillar-card.expandable')) {
                const pillar = e.target.closest('.pillar-card');
                const pillarTitle = pillar?.querySelector('.pillar-title')?.textContent || 'unknown';
                const isExpanding = !pillar?.classList.contains('expanded');
                trackInteraction('pastoral_care', isExpanding ? 'pillar_expand' : 'pillar_collapse', pillarTitle);
            }

            // Pastoral care - journey timeline steps
            if (e.target.classList.contains('journey-step')) {
                const stepNumber = Array.from(e.target.parentElement.children).indexOf(e.target) + 1;
            ...
            if (e.target.classList.contains('sport-tab')) {
                const sportType = e.target.getAttribute('data-sport') || e.target.textContent.trim();
                trackInteraction('sports', 'sport_tab_click', sportType);
            }

            // Sports - unmute buttons
