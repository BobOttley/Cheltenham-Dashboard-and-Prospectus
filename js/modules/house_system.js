MODULES['house_system'] = (root, ctx) => {
  ...
  root.querySelectorAll('.child-name').forEach(el => {
    const fallback = el.getAttribute('data-fallback') || t('house_system.placeholders.your_child');
  ...
  if (note) {
    if (ctx?.priorities?.pastoral === 3) {
      note.textContent = t('house_system.notes.pastoral_high', { childName: ctx.childName || t('house_system.placeholders.your_child') });
    } else {
  ...
  root.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('mute-button')) {
      ...
      if (video.muted) {
        ...
        video.muted = false;
        video.volume = 1.0;
        e.target.innerHTML = t('house_system.ui.click_to_mute');
        ...
        // Mute
        video.muted = true;
        e.target.innerHTML = t('house_system.ui.click_for_sound');
    ...
    const checkVideoVisibility = () => {
      ...
      expandedCards.forEach(card => {
        ...
        if (!isVisible && !video.muted) {
          ...
          if (btn) btn.innerHTML = t('house_system.ui.click_for_sound');
