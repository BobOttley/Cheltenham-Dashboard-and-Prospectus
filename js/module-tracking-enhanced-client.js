...
(function() {
    ...
    function setupInteractionTracking() {
        ...
        document.addEventListener('click', (e) => {
            ...
            // ========================================
            // HERO MODULE - Audio Toggle Button
            // ========================================
            const heroModule = e.target.closest('.mod--hero, .hero-module');
            if (heroModule) {
                // Audio toggle button - check ID and classes
                if (e.target.id === 'audioToggle' ||
                    e.target.classList.contains('audio-toggle') || 
                    e.target.closest('#audioToggle, .audio-toggle')) {
                    
                    const video = heroModule.querySelector('.hero-video, video');
            ...
            // ========================================
            // FINAL HERO MODULE - Audio Toggle Button
            // ========================================
            const finalHeroModule = e.target.closest('.mod--final_hero, .mod--final-hero, .fh-final-module');
            if (finalHeroModule) {
                if (e.target.id === 'audioToggle' ||
                    e.target.classList.contains('audio-toggle') || 
                    e.target.classList.contains('fh-unmute-btn') ||
                    e.target.closest('#audioToggle, .audio-toggle, .fh-unmute-btn')) {
                    
                    const video = finalHeroModule.querySelector('.fh-hero-video, video');
                    const isMuting = video && !video.muted;
            ...
            // ========================================
            // CCF MODULE - Audio Toggle Button
            // ========================================
            ...
            if (ccfModule) {
                ...
                if (e.target.id === 'ccfAudioToggle' ||
                    e.target.classList.contains('audio-toggle') ||
                    e.target.closest('#ccfAudioToggle, .audio-toggle')) {
            ...
            // ========================================
            const ctaButton = e.target.closest('.cta-button, .book-visit-btn, .visit-btn, .come-see-us-btn, .ccf-cta-button, .fh-cta-button, .sp-cta-button');
            if (ctaButton) {
                ...
                if (parentModule) {
                    const classList = Array.from(parentModule.classList);
                    const modClass = classList.find(c => c.startsWith('mod--'));
                    if (modClass) {
                        moduleName = modClass.replace('mod--', '');
                    }
                } else {
                    // Check if in specific sections
                    if (ctaButton.closest('.cta-section')) moduleName = 'hero';
                    else if (ctaButton.closest('.ccf-cta-section')) moduleName = 'ccf';
                    else if (ctaButton.closest('.fh-cta-section')) moduleName = 'final_hero';
                }
            ...
            if (upperSchoolModule) {
                // Check for section filter buttons - multiple selector strategies
                const sectionButton = e.target.closest('.us-nav-btn, [data-filter]') || 
                                    (e.target.classList.contains('us-nav-btn') ? e.target : null) ||
                                    (e.target.hasAttribute('data-filter') ? e.target : null);
                
                if (sectionButton) {
                    const section = sectionButton.getAttribute('data-filter') || 
            ...
            if (sciencesModule) {
                const subjectButton = e.target.closest('.sm-nav-btn, [data-filter]') || 
                                    (e.target.classList.contains('sm-nav-btn') ? e.target : null) ||
                                    (e.target.hasAttribute('data-filter') ? e.target : null);
                
                if (subjectButton) {
                    const subject = subjectButton.getAttribute('data-filter') || 
                                  subjectButton.getAttribute('data-subject') ||
            ...
            if (humanitiesModule) {
                const subjectButton = e.target.closest('.hum-nav-btn, [data-filter]') || 
                                    (e.target.classList.contains('hum-nav-btn') ? e.target : null) ||
                                    (e.target.hasAttribute('data-filter') ? e.target : null);
            ...
            if (languagesModule) {
                const subjectButton = e.target.closest('.lc-nav-btn, [data-filter]') || 
                                    (e.target.classList.contains('lc-nav-btn') ? e.target : null) ||
                                    (e.target.hasAttribute('data-filter') ? e.target : null);
            ...
            if (creativeArtsModule) {
                const subjectButton = e.target.closest('.nav-btn, [data-filter]') || 
                                    (e.target.classList.contains('nav-btn') ? e.target : null) ||
                                    (e.target.hasAttribute('data-filter') ? e.target : null);
            ...
            if (appliedModule) {
                const subjectButton = e.target.closest('.av-nav-btn, [data-filter]') || 
                                    (e.target.classList.contains('av-nav-btn') ? e.target : null) ||
                                    (e.target.hasAttribute('data-filter') ? e.target : null);
