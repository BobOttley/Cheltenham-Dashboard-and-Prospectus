 ...
 * Loads language-specific HTML files and CSS for modules based on PROSPECTUS_LANG
 */
...
(function() {
  ...
  // Function to load CSS for a module
  async function loadModuleCSS(moduleName) {
    // Check if CSS already loaded
    if (document.querySelector(`link[data-module="${moduleName}"]`)) {
      console.debug(`[i18n] CSS already loaded for: ${moduleName}`);
      return;
    }
    ...
    try {
      ...
      if (cssCheck.ok) {
        ...
        console.log(`[i18n] ✓ Loaded CSS for: ${moduleName}`);
      } else {
        console.debug(`[i18n] No CSS file for: ${moduleName}`);
      }
    } catch (error) {
      console.debug(`[i18n] CSS check failed for ${moduleName}:`, error.message);
    }
  }

  // Replace placeholder with translated HTML and load CSS
  async function loadPlaceholder(placeholder, lang) {
    ...
    try {
      ...
      const [html] = await Promise.all([
        loadModuleHTML(moduleName, lang),
      ...
      console.debug(`[i18n] ✓ Module ready: ${moduleName}`);
    } catch (error) {
