...
app.get('/api/enquiry/:id', async (req, res) => {
    try {
        ...
        console.error('âŒ Error fetching enquiry:', error);
...
app.get('/prospectus', async (req, res) => {
    ...
    try {
        ...
        if (targetLang && targetLang !== 'en') {
            // For now, just send English - proper HTML translation needs DeepL Document API
            console.log('Translation requested but not implemented:', targetLang);
        }

        res.send(renderedHTML);
        
    } catch (error) {
        console.error('âŒ Error rendering prospectus:', error);
...
// Translation endpoint// Batch translation endpoint
...
app.post('/api/translate', async (req, res) => {
    try {
      ...
      const rawLang = (targetLang || '').toString().trim().toLowerCase();
  
      // Map common UI codes to DeepL codes (expand as needed)
      const LANG_MAP = {
        en: 'EN',
        fr: 'FR',
        es: 'ES',
        ...
        nl: 'NL',
        pl: 'PL',
        pt: 'PT-PT',    // or PT-BR if you need Brazilian Portuguese
        ru: 'RU',
        ja: 'JA',
        ko: 'KO',
        zh: 'ZH'        // DeepL expects ZH
        // ar: (not currently supported by DeepL; omit)
      };
  
      const deeplLang = LANG_MAP[rawLang];
      if (!deeplLang) {
        return res.status(400).json({ success: false, error: `Unsupported target language: ${targetLang}` });
      }
  
      // Clean the inputs: drop empties/whitespace-only
      const items = texts.map((t, i) => ({ i, t: (t || '').toString() }))
      ...
      // Batch to avoid oversized requests (safe defaults)
      const MAX_ITEMS = 40;
      ...
      while (cursor < items.length) {
        let count = 0, chars = 0, batch = [];
        while (cursor + count < items.length) {
          const candidate = items[cursor + count];
          if (batch.length >= MAX_ITEMS) break;
          if (chars + candidate.t.length > MAX_CHARS) break;
          batch.push(candidate);
          chars += candidate.t.length;
          count++;
        }
  
        // Build form-encoded payload
        const params = new URLSearchParams();
        batch.forEach(b => params.append('text', b.t));
        params.append('target_lang', deeplLang);
        params.append('preserve_formatting', '1');      // keep spaces/line breaks
        params.append('split_sentences', 'nonewlines'); // don’t split on \n
        ...
        const response = await axios.post(
          ...
          params,
          {
            headers: {
              ...
              // DO NOT set Content-Type manually; axios assigns right headers for URLSearchParams
            },
            timeout: 20000
          }
        );
  
        const translated = (response.data?.translations || []).map(x => x.text);
  
        // Put results back in original order
        translated.forEach((txt, idx) => {
          const originalIndex = batch[idx].i;
          allTranslations[originalIndex] = txt;
        });
  
        cursor += count;
      }
  
      return res.json({ success: true, translations: allTranslations });
    } catch (err) {
      // Show more helpful detail if DeepL returned a body
      const status = err.response?.status;
      const body = err.response?.data;
      console.error('❌ Translation error:', status, body || err.message);
      return res.status(500).json({
        success: false,
        error: status ? `DeepL ${status}` : err.message
      });
    }
  });
