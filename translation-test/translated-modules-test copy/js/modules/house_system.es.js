MODULES['house_system'] = (root, ctx) => {
  ...
  root.querySelectorAll('.child-name').forEach(el => {
    const fallback = el.getAttribute('data-fallback') || "Su hijo";
  ...
  if (note) {
    if (ctx?.priorities?.pastoral === 3) {
      note.textContent = "Me alegro de que valore tanto la atención pastoral: el bienestar de ${ctx.childName || 'your child'} será siempre nuestra prioridad.";
    } else {
  ...
  console.log("Filtro de casa - Género:", gender, 'Boarding:', pref);
  ...
  const loadVideoSource = (video) => {
    ...
    video.src = dataSrc;
    video.load();
    console.log("Vídeo cargado de pereza:", dataSrc);
  };
  ...
  const playVideo = (video) => {
    ...
    setTimeout(() => {
      video.play().catch(err => {
        console.log("Bloqueo de la reproducción automática de vídeo:", err);
      });
    }, 100);
  };
  ...
  root.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('mute-button')) {
      ...
      if (video.muted) {
        ...
        video.muted = false;
        video.volume = 1.0;
        e.target.innerHTML = "🔇 Haz clic para silenciar";
        console.log("Vídeo doméstico sin silenciar");
        
        // If paused, play it
        if (video.paused) {
          video.play().catch(err => {
            console.error("No se puede reproducir el vídeo:", err);
          });
        }
      } else {
        // Mute
        video.muted = true;
        e.target.innerHTML = "🔊 Haz clic para ver el sonido";
        console.log("Vídeo de la Cámara silenciado");
      }
      return;
    }
    ...
    const checkVideoVisibility = () => {
      ...
      expandedCards.forEach(card => {
        ...
        if (!isVisible && !video.muted) {
          ...
          if (btn) btn.innerHTML = "🔊 Haz clic para ver el sonido";
          console.log("El vídeo de la casa se silencia automáticamente - se pierde de vista");
        }
      });
    };
