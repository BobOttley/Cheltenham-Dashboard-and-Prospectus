...
pool.query("SET timezone = 'Europe/London'");
...
// NOW register routes AFTER session middleware
...
// Redirect old auth callback to new one
app.get('/auth/callback', (req, res) => {
    const queryString = new URLSearchParams(req.query).toString();
    res.redirect(`/api/msauth/callback?${queryString}`);
});
...
function generateParentEmailDraft(enquiryData, prospectusUrl) {
    const {
        ...
        parentName = 'there',
        email: parentEmail = '',
        family_surname = '',      // ADD THIS
        familySurname = '',       // ADD THIS (fallback)
        formData = {}
    } = enquiryData;
    ...
    // ADD THIS LINE - Get family surname reliably
    const familyName = family_surname || familySurname || parentLastName || '';
...
If you have any questions – big or small – please do just pick up the phone or drop me an email. I'm here to help, and I'd love to hear from you.`;
    ...
    const parts = [
        `Dear ${familyName ? 'Mr/Mrs ' + familyName : parentFirstName},`,
        '',
        openingPara,
        '',
        mainContent,
    ...
    const htmlParts = [
        `<p>Dear ${familyName ? 'Mr/Mrs ' + familyName : parentFirstName},</p>`,
        `<p>${openingPara}</p>`,
        ...mainContent.split('\n\n').map(p => `<p>${p}</p>`),
...
<body>
    ...
    return {
        to: parentEmail,
        subject: `Welcome ${firstName} and the ${familyName} Family to Cheltenham College`,
        textContent,
        htmlContent
    };
}
...
function transformFormData(formData) {
    ...
    familyName: (() => {
        // If familyName is provided
        if (formData.familyName) {
            const fn = formData.familyName.trim();
            // If it already has "the" and "family", use as-is
            if (fn.toLowerCase().startsWith('the ') && fn.toLowerCase().includes(' family')) {
                return fn;
            }
            // Otherwise, format it properly
            return `the ${fn} family`;
        }
        // Fallback to extracting from parentName
        if (formData.parentName) {
            return `the ${formData.parentName.split(' ').pop()} family`;
        }
        return '';
    })(),
...
app.post('/api/admin/enquiries/create', requireAuth, async (req, res) => {
    try {
        const {
            first_name,
            family_surname,
            ...
            academic_interests,
            activities,
            specific_sports,  // Now an array from sport1, sport2, sport3
            additional_info
        } = req.body;

        // Validation
        if (!first_name || !family_surname || !parent_name || !parent_email) {
            return res.status(400).json({
                success: false,
                error: 'Required fields missing: first_name, family_surname, parent_name, parent_email'
            });
        }
        ...
        // Build form_data JSON object - MATCHING ENQUIRY FORM STRUCTURE
        const formData = {
            stage: stage || '',  // Lower or Upper
            gender: gender || '',
            boardingPreference: boarding_preference || '',
            academicInterests: academic_interests || [],  // Array of selected interests
            activities: activities || [],
            specificSports: specific_sports || [],  // Array from sport1, sport2, sport3
            universityAspirations: university_aspirations || '',
        ...
        // NOTE: school is ALWAYS set to 'cheltenham' - this is the default school value
        await pool.query(
            ...
            [
                ...
                form_entry || '',
                'cheltenham',  // ALWAYS 'cheltenham' - required default
        ...
        try {
            ...
            console.log('Activity log not available (table may not exist)');
        }

        console.log(`New enquiry created: ${enquiryId} for ${first_name} ${family_surname}`);
...
// REPLACE YOUR EXISTING app.patch('/api/admin/enquiries/:id') in server.js WITH THIS:
...
app.patch('/api/admin/enquiries/:id', requireAuth, async (req, res) => {
    try {
        ...
        // Columns that are ACTUAL database columns
        const directColumns = [
            'status', 'priority', 'followup_date', 'email_sent',
            'first_name', 'family_surname', 'parent_email', 'parent_name',
            'contact_number', 'country', 'age_group', 'entry_year', 
            'form_entry', 'school', 'prospectus_generated'
        ];
        
        // Fields that are stored INSIDE form_data JSON
        const formDataFields = [
            'gender', 'stage', 'boardingPreference', 'boarding_preference', 'academicInterests', 
            'activities', 'specificSports', 'universityAspirations', 'priorities'
        ];
        ...
        // Separate direct column updates from form_data updates
        for (const [key, value] of Object.entries(req.body)) {
            if (directColumns.includes(key)) {
                ...
                if (key === 'status') {
                    await logActivity(id, 'status_changed', `Status changed to ${value}`, adminEmail);
                } else if (key === 'priority') {
                    await logActivity(id, 'priority_changed', `Priority ${value ? 'enabled' : 'disabled'}`, adminEmail);
                } else if (key === 'followup_date') {
                    await logActivity(id, 'followup_set', `Follow-up set for ${value}`, adminEmail);
                }
            } else if (formDataFields.includes(key)) {
                // Map boarding_preference to boardingPreference for consistency
                const formDataKey = key === 'boarding_preference' ? 'boardingPreference' : key;
                formDataUpdates[formDataKey] = value;
            }
        }
        
        // If there are form_data updates, get current form_data and merge
        if (Object.keys(formDataUpdates).length > 0) {
            const currentData = await pool.query('SELECT form_data FROM inquiries WHERE id = $1', [id]);
            if (currentData.rows.length > 0) {
                const currentFormData = currentData.rows[0].form_data || {};
                const mergedFormData = { ...currentFormData, ...formDataUpdates };
                
                updates.push(`form_data = $${paramCount}`);
                values.push(JSON.stringify(mergedFormData));
                paramCount++;
            }
        }
        ...
        console.log(`✅ Updated enquiry ${id}:`, Object.keys(req.body).join(', '));
